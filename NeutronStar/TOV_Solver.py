...
try:
    ...
    print(
        "Unable to import matplotlib.pyplot @ TOV_Solver. Make sure that\
        the package is installed to the current env to use plot module."
    )
...
class Star(EoS):
    ...
    def __init__(self, barP0=0.0):
        ...
        self.__r0 = self.__dr
        self.barP0 = barP0
        # Central values of the star.
        cent_values = self.get_star_central_values()
...
# ------------------------------------------------------------------------------------------------------------
    ...
    def print_central_values(self):
        ...
        for item, value in star_central_values.items():
            print(" ", item.ljust(left_width, '.')+" | " +
                  '{:8.6e}'.format(value).ljust(rigt_width))
        print("\n")
...
# ------------------------------------------------------------------------------------------------------------
    ...
    def TOV_solver(self):
        ...
        outfname = os.path.join(output_folder, f".Star_barP0_{self.barP0:4.4}.dat")
        ...
        while(True):
            ...
            if barP_lst+(0.5*barPK1) > 0.0:
                break_loop = False
                barPK2 = dr * \
                    self.dbarP_dr(r_lst+(0.5*dr), barM_lst,
                                  barP_lst+(0.5*barPK1))
            else:
                break_loop = True
                break
            ...
            if barP_lst+barPK3 >= 1.0e-12:
                break_loop = False
                barPK4 = dr * \
                    self.dbarP_dr(r_lst+dr, barM_lst, barP_lst+barPK3)
            else:
                break_loop = True
                break
            # ------------------------------------------------------------------------------------------
            ...
            barMK2 = dr * \
                self.dbarM_dr(r_lst+(0.5*dr), barP_lst, barM_lst+(0.5*barMK1))
            barMK3 = dr * \
                self.dbarM_dr(r_lst+(0.5*dr), barP_lst,
                              barM_lst + (0.5*barMK2))
            barMK4 = dr * self.dbarM_dr(r_lst+dr, barP_lst, barM_lst+barMK3)

            # ------------------------------------------------------------------------------------------

            barP_nxt = barP_lst + (1.0/6.0) * \
                (barPK1+(2.0*barPK2)+(2.0*barPK3)+barPK4)
            barM_nxt = barM_lst + (1.0/6.0) * \
                (barMK1+(2.0*barMK2)+(2.0*barMK3)+barMK4)
...
def main():
    ...
    RVsbarM_fname = os.path.join(output_folder, "RVsbarM.dat")
