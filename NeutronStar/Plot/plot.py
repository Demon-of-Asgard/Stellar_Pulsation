...
class Plot:
    ...
    def double_plot(self,filename):
        ...
        ax[0].plot(data[:, 0]/1.0e5, data[:, 1], color=colors["royal_blue"], linestyle="-", linewidth = lw)
        ...
        ax[1].plot(data[:, 0]/1.0e5, data[:, 2], color=colors["salmon"], linestyle="-", linewidth = lw)
...
#-------------------------------------------------------------------------------------------------
    ...
    def single_plot(self, filename, title="R${_{NS}}$ Vs M/M${_{\\odot}}$"):
            ...
            ax.plot(data[:, 0]/1.0e5, data[:, 1], color=colors["salmon"],
                    linestyle="-", zorder = 1, linewidth=lw)

            ax.scatter(data[:, 0]/1.0e5, data[:, 1], color=colors["royal_blue"],
            ...
            plt.savefig("mVsR.jpeg", dpi = 300, bbox_inches = "tight")
